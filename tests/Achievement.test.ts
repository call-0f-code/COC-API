import {
  ...
  removeMemberFromAchievement,
} from "../src/controllers/achievement.controller";
...
describe("createAchievement (with image upload)", () => {
  it("should return 201 and created achievement with uploaded image URL", async () => {
    ...
    const mockAchievement = {
      ...
      updatedAt: new Date(),
      updatedById: null,
      members: [{ memberId: "user_1" }, { memberId: "user_2" }],
    };

    mockedUploadImage.mockResolvedValue(imageUrl);
    jest
      .spyOn(achievementService, "createAchievement")
...
describe("getAchievements", () => {
  it("should return 200 and all achievements", async () => {
    ...
    jest
      .spyOn(achievementService, "getAchievements")
...
describe("updateAchievementById", () => {
  ...
  it("should update all fields successfully (200)", async () => {
    ...
    jest
      .spyOn(achievementService, "getAchievementById")
      .mockResolvedValue(baseAchievement);
    jest
      .spyOn(achievementService, "updateAchievementById")
      .mockResolvedValue(updatedAchievement);
    jest
      .spyOn(achievementService, "addMembersToAchievement")
  ...
  it("should update only title", async () => {
    ...
    await updateAchievementById(mockReq, mockRes);
    expect(achievementService.updateAchievementById).toHaveBeenCalledWith(
      1,
  ...
  it("should update only memberIds", async () => {
    ...
    await updateAchievementById(mockReq, mockRes);
    expect(achievementService.addMembersToAchievement).toHaveBeenCalledWith(1, [
      "user_3",
    ]);
  });
  ...
  it("should return 400 if updatedById is missing", async () => {
    ...
    await expect(updateAchievementById(mockReq, mockRes)).rejects.toThrow(
      ApiError
    );
  });
...
describe("deleteAchievementById", () => {
  it("should delete the achievement and return 200 success message", async () => {
    ...
    expect(mockedDeleteImage).toHaveBeenCalledWith(
      expect.anything(),
      mockDeletedAchievement.imageUrl
    );
...
describe("removeMemberFromAchievement", () => {
  ...
  it("should throw 400 for invalid achievement ID", async () => {
    ...
    await expect(removeMemberFromAchievement(req, res)).rejects.toThrow(
      ApiError
    );
  });
