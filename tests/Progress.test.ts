import {getCompletedQuestion,toggleQuestions} from '../src/controllers/progress.controller';
...
describe("get coompleted questions for a member",()=>{
    ...
    it("should throw 400 for missing required field",async()=>{
        ...
        await expect(getCompletedQuestion(req,res)).rejects.toThrow(new ApiError("required field is missing",400));
    })
})
...
describe("toggle questions",()=>{
    it("should respond with 200 and toggle completed question ", async()=>{
        ...
        const res = mockResponse();
        const spy = jest.spyOn(progressServices,'markQuestion').mockResolvedValue(baseMockCompletedQuestion);
        await toggleQuestions(req,res);
        expect(spy).toHaveBeenCalledWith(parseInt(req.params.questionId),req.params.memberId);
        expect(res.status).toHaveBeenCalledWith(200);
    ...
    it("should throw 400 for missing required field",async()=>{
        ...
        const res = mockResponse();
        await expect(toggleQuestions(req,res)).rejects.toThrow(new ApiError("required field is missing",400));
    })
})